...
class VoiceAssistant(QObject):
    def __init__(self, config_path="config.json", user_callback=None, ai_callback=None, communicator=None):
        ...
        if self.communicator:
            self.communicator.sleep_mode_changed.connect(self.set_sleep_mode)

    @pyqtSlot(bool)
    def set_sleep_mode(self, sleep):
        """设置睡眠模式"""
        print(f"设置睡眠模式: {'睡眠' if sleep else '唤醒'}")  # 添加调试信息
        self.sleep_mode = sleep
        if sleep:
            # AI 进入睡眠模式，发声提示
            self.generate_and_play_tts("我先休息了，有事叫我哦。", self.emotion_mapping.get('NEUTRAL', 'lovey-dovey'))
        else:
            # AI 退出睡眠模式，发声提示
            self.generate_and_play_tts("我醒啦，有什么吩咐呢？", self.emotion_mapping.get('NEUTRAL', 'lovey-dovey'))
    ...
    def update_config(self, new_config):
        ...
        self.LOG_FILE_PATH = self.config.get("log_file_path", "log/conversation.log")
        self.MAX_LOG_ENTRIES = self.config.get("history_limit", 1000)
        self.wake_up_word = self.config.get("wake_up_word", "")
...
"""
    ...
    def process_user_input(self, user_input, emotion='NEUTRAL', is_voice=False):
        ...
        with self.input_lock:
            if self.sleep_mode:
                if not is_voice:
                    # 用户通过UI发送消息，作为唤醒条件
                    print("检测到来自UI的消息，唤醒AI。")
                    self.set_sleep_mode(False)  # 退出休眠模式
                else:
                    wake_up_word = self.config.get('wake_up_word', '')
                    if wake_up_word and wake_up_word in user_input:
                        # 检测到唤醒词，触发唤醒
                        print("检测到唤醒词，唤醒AI。")
                        self.set_sleep_mode(False)  # 退出睡眠模式
                        return
                    else:
                        # 在睡眠模式下，不处理其他语音输入
                        print("AI处于睡眠模式，忽略该语音输入。")
                        return
            ...
            if actions:
                ...
                # **新增部分：将 tts_message 添加到对话历史和日志，并发送到UI**
                self.messages.append({"role": "assistant", "content": tts_message})
                self.append_to_conversation_log({"role": "assistant", "content": tts_message})
                if len(self.messages) > self.MAX_LOG_ENTRIES:
                    self.messages.pop(0)

                if self.ai_callback:
                    self.ai_callback(tts_message)  # 通过AI回调发送消息到UI
                ...
                for action in actions:
                    ...
                    if skill_name == "web_search":
                        ...
                        result = self.execute_bilibili_open(input_text)
                    else:
                        # 处理未定义的技能
                    ...
                    if result.get('type') == 'text':
                        ...
                        assistant_response_parts.append(f"图像生成好啦，请查看")
    ...
    def execute_bilibili_open(self, query):
        """执行打开特定网页"""
        import webbrowser
        try:
            webbrowser.open("https://www.bilibili.com/")
            return {"type": "text", "content": "已为您打开B站。"}
        except Exception as e:
            return {"type": "error", "content": f"打开B站时出错: {e}"}
    ...
    def generate_and_play_tts(self, text, tts_emotion):
        ...
        try:
            ...
            if "data" in response_data:
                ...
                with open(mp3_filename, "wb") as file_to_save:
