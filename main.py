...
def load_config():
    ...
    if not os.path.exists(CONFIG_FILE):
        ...
        roles = {
            ...
            "胖熊猫": {
                "gif1": "assets/role3/gif1.gif",
                "gif2": "assets/role3/gif2.gif",
                "voice_type": "BV050_streaming"
            },
            "棕色妹妹": {
                "gif1": "assets/role4/gif1.gif",
                "gif2": "assets/role4/gif2.gif",
                "voice_type": "BV700_V2_streaming"
            },
            "性感姐姐": {
                "gif1": "assets/role5/gif1.gif",
                "gif2": "assets/role5/gif2.gif",
                "voice_type": "BV428_streaming"
            }
        }
        default_role = list(roles.keys())[0]  # 获取第一个角色
        default_config = {
            "volcano_api_key": {
            ...
            "voice_tone": roles[default_role].get("voice_type", "lovey-dovey"),
            "log_file_path": "log/conversation.log",
            "history_limit": 1000,
            "user": {
                "uid": "2101710118"
            },
            "voice_type": roles[default_role].get("voice_type", "BV700_V2_streaming"),
            "roles": roles,
        ...
        if not roles:
            QMessageBox.critical(None, "错误", "配置文件中未定义任何角色。")
            sys.exit(1)
        ...
        # 检查当前 selected_role 是否在角色列表中
        selected_role = config.get("selected_role")
        if selected_role not in roles:
            # 如果 selected_role 无效，设置为第一个角色
            config["selected_role"] = role_names[0]
            # 同时更新 voice_type
            config["voice_type"] = roles[role_names[0]].get("voice_type", "BV700_V2_streaming")
            save_config(config)
            QMessageBox.information(None, "信息", f"selected_role 未找到，已设置为第一个角色：{role_names[0]}")
...
class MainWindow(QWidget):
    def __init__(self, config, communicator, assistant):
        ...
        self.selected_role = self.config.get("selected_role", "粉色妹妹")
        self.load_role_gifs()
